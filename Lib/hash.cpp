...
int hashTable::insert(const std::string &key, void *pv) {
  ...
  while (v[index] != nullptr && v[index]->isOccupied &&
    ...
    // deleted If it is, we can safely fill that square As a result, when
    // you keep filling up the hash table and keep removing all the
    // elements, it will crash. Hopefully this fixes things
    if (v[index]->key == key) {
      return 1;
    }
    index++;
