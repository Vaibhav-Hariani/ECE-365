...
int hashTable::insert(const std::string &key, void *pv) {
  ...
  while (v[index] != nullptr && v[index]->isOccupied &&
         !(v[index]->isDeleted)) {
    // This was likely the bug: I wasn't checking to see if the element was
    // deleted If it is, we can safely fill that square As a result, when you
    // keep filling up the hash table and keep removing all the elements, it
    // will crash. Hopefully this fixes things
    if (v[index]->key == key) {
      return 1;
    }
    index++;
...
unsigned int hashTable::getPrime(int size) {
  const unsigned int good_primes[19] = {
      1543,      6151,      24593,     49157,     98317,     196613,   393241,
